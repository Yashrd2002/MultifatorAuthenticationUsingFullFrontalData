...
//     try {
//       const q =  query(
//         collection(db, `user`), where(`user.email`, "==", location?.state?.account.email)
//       );
...
// },[])
  ...
  useEffect(() => {
    ...
    getDownloadURL(ref(storage, `images/${location?.state?.account.email}`)).then((url) => {
      ...
      console.log(tempAccount); // This might not show the updated state due to asynchronicity
    })
    .catch((error) => {
      console.log(error);
    });
